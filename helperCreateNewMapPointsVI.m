function [G, pointToNode, mapPoints, vSetKeyFrames, recentPointIdx] = helperCreateNewMapPointsVI(...
    G, pointToNode, viewToNode,...
...
for i = 1:numel(KcIDs)
    ...
    if any(inlier)
        ...
        ptsIds = generateNodeID(G,height(xyzPoints))';
        pointToNode = [pointToNode;ptsIds];

        poseIds=ones(height(xyzPoints),1)*viewToNode(KcIDs(i));
        ptsInView=vSetKeyFrames.Views.Points{KcIDs(i),1}.Location;
        mesPts=ptsInView(mIndices1,:);
        fCam=factorCameraSE3AndPointXYZ([poseIds ptsIds], K, Measurement=mesPts, ...
        Information=camInfo);
        addFactor(G,fCam);

        poseIds=ones(height(xyzPoints),1)*viewToNode(currKeyFrameId);
        ptsInView=vSetKeyFrames.Views.Points{currKeyFrameId,1}.Location;
        mesPts=ptsInView(mIndices2,:);
        fCam=factorCameraSE3AndPointXYZ([poseIds ptsIds], K, Measurement=mesPts, ...
        Information=camInfo);
        addFactor(G,fCam);

        nodeState(G,ptsIds,double(xyzPoints));
    end
end
end
